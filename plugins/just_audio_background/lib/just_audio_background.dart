...
class JustAudioBackground {
  /// Actions used by the app-specific Android notification controls.
  static const notificationFavoriteAction =
      'just_audio_background.notification.favorite';
  static const notificationCloseAction =
      'just_audio_background.notification.close';
  static const notificationSetFavoriteAction =
      'just_audio_background.notification.set_favorite';
  static const notificationEventType =
      'just_audio_background.notification.action';
  ...
  /// Updates the heart icon without interrupting playback.
  static Future<void> setNotificationFavorite(bool isFavorite) =>
      _audioHandler.customAction(notificationSetFavoriteAction, {
        'isFavorite': isFavorite,
      });

  /// Events emitted by app-specific controls in the media notification.
  static Stream<dynamic> get notificationActionStream =>
...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  Future<dynamic> customAction(
    ...
    if (name == JustAudioBackground.notificationSetFavoriteAction) {
      _isFavorite = extras?['isFavorite'] == true;
      _broadcastStateIfActive();
      return null;
    }
    if (name == JustAudioBackground.notificationFavoriteAction ||
        name == JustAudioBackground.notificationCloseAction) {
      customEvent.add({
        'type': JustAudioBackground.notificationEventType,
        'action': name,
      });
      return null;
    }
  ...
  void _broadcastState() {
    final controls = [
      MediaControl.custom(
        ...
        name: JustAudioBackground.notificationFavoriteAction,
      ),
    ...
    playbackState.add(playbackState.nvalue!.copyWith(
      ...
      // Android compact notifications show only three actions. The expanded
      // player keeps the favorite and close controls at both ends.
